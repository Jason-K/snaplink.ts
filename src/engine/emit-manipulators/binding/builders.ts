...
export function buildMultiTap(
  ...
  if (isSim) {
    ...
    const manipulators = simultaneousMultiTap({
      ...
      simultaneousThresholdMs: resolveSimThreshold(b),
    });
    const pressDo = cases
...
function resolveSimThreshold(b: Binding): number | undefined {
  return (
    b.timing?.simultaneousMs ??
    b.simultaneousMs ??
    b.thresholdMs ??
    ("chordTiming" in b.trigger ? b.trigger.chordTiming?.simultaneousMs : undefined)
  );
}
...
export function buildSimultaneousTapHold(
  ...
  const simThreshold = resolveSimThreshold(b);
  ...
  const manipulators = hasTapHold
    ? simultaneousTapHold({
        ...
        karOptions: resolveSimKarOptions(b),
        simultaneousThresholdMs: simThreshold,
      })
    : simultaneousRemap({
        keys,
        to: pressEvents,
