...
export function normalizeSimOrder(
  ...
  if (order === "uninterrupted") return { detectUninterrupted: true };
  ...
  if ("detectUninterrupted" in order && order.detectUninterrupted !== undefined)
    o.detectUninterrupted = Boolean(order.detectUninterrupted);
  if ("detect_key_down_uninterruptedly" in order && order.detect_key_down_uninterruptedly !== undefined)
    o.detectUninterrupted = Boolean(order.detect_key_down_uninterruptedly);
  if ("uninterrupted" in order && (order as { uninterrupted?: boolean }).uninterrupted !== undefined)
    o.detectUninterrupted = Boolean((order as { uninterrupted?: boolean }).uninterrupted);
...
export function trigger(
  ...
  order?: SimOrder | "insensitive" | "strict" | "strict_inverse" | "uninterrupted",
): Trigger {
  ...
  return triggerKeys(input, modifiers, order);
}
 ...
 * @param order - Optional simultaneous key ordering constraint (`"insensitive"`, `"strict"`, `"strict_inverse"`, `"uninterrupted"`, or {@link SimOrder}).
 * @param timing - Optional chord timing thresholds.
...
export function triggerKeys(
  ...
  if (keysArray.length > 1) {
    return createSimultaneousTrigger(keysArray, modifiers, resolvedOrder, timing);
  }
  return {
    keys: keysArray,
    ...(modifiers ? { modifiers } : {}),
...
export type SimultaneousTriggerOptions = {
  ...
  /**
   * Key press down/up ordering constraint (`"insensitive"`, `"strict"`, `"strict_inverse"`, `"uninterrupted"`, or full {@link SimOrder}).
   *
   * @example "strict"
  ...
  /**
   * Shorthand for `detect_key_down_uninterruptedly: true`.
   */
  detect_key_down_uninterruptedly?: boolean;

  /**
   * Alias for `detect_key_down_uninterruptedly`.
   */
  ...
  /**
   * Chord threshold window in milliseconds (`basic.simultaneous_threshold_milliseconds`).
   */
  simultaneousMs?: number;

  /**
   * Alias for `simultaneousMs`.
   */
  thresholdMs?: number;
  ...
  /**
   * Timing configuration in ms or {@link BindingTiming} object.
   */
  timing?: number | BindingTiming;
};

/**
 * A simultaneous multi-key chord {@link Trigger} augmented with fluent builder methods.
 */
export type SimultaneousTrigger = Trigger & {
  /**
   * Mark chord key detection uninterrupted (unrelated keys between chord keys will not cancel match).
   */
  uninterrupted(val?: boolean): SimultaneousTrigger;

  /**
   * Alias for {@link uninterrupted}.
   */
  detectKeyDownUninterruptedly(val?: boolean): SimultaneousTrigger;

  /**
   * Set simultaneous threshold window in ms (or explicit timing parameters).
   */
  timing(msOrTiming: number | TimingProfileName | BindingTiming): SimultaneousTrigger;

  /**
   * Set simultaneous threshold window in milliseconds.
   */
  threshold(ms: number): SimultaneousTrigger;

  /**
   * Enforce strict key press down order (`key_down_order: "strict"`).
   */
  strict(): SimultaneousTrigger;

  /**
   * Set ordering constraint.
   */
  withOrder(order: SimOrder | "insensitive" | "strict" | "strict_inverse" | "uninterrupted"): SimultaneousTrigger;

  /**
   * Set modifier requirements for the chord.
   */
  withModifiers(mods: TriggerModifiers): SimultaneousTrigger;
};

export function createSimultaneousTrigger(
  keys: TriggerKey[],
  modifiers?: TriggerModifiers,
  order?: SimOrder,
  chordTiming?: BindingTiming,
): SimultaneousTrigger {
  const trg = {
    keys,
    ...(modifiers ? { modifiers } : {}),
  ...
  } as SimultaneousTrigger;

  Object.defineProperties(trg, {
    uninterrupted: {
      value(this: SimultaneousTrigger, val = true) {
        const currOrder = "order" in this && this.order ? this.order : {};
        (this as { order?: SimOrder }).order = { ...currOrder, detectUninterrupted: val };
        return this;
      },
      enumerable: false,
      writable: true,
    ...
    timing: {
      value(this: SimultaneousTrigger, msOrTiming: number | TimingProfileName | BindingTiming) {
        const currTiming = "chordTiming" in this && this.chordTiming ? this.chordTiming : {};
        if (typeof msOrTiming === "number") {
          (this as { chordTiming?: BindingTiming }).chordTiming = { ...currTiming, simultaneousMs: msOrTiming };
        } else if (isTimingProfileName(msOrTiming)) {
          (this as { chordTiming?: BindingTiming }).chordTiming = { ...currTiming, ...TIMING_PROFILES[msOrTiming] };
        } else {
          (this as { chordTiming?: BindingTiming }).chordTiming = { ...currTiming, ...msOrTiming };
        }
        return this;
      },
      enumerable: false,
    ...
    threshold: {
      value(this: SimultaneousTrigger, ms: number) {
        return this.timing(ms);
      },
      enumerable: false,
      writable: true,
      configurable: true,
    ...
    strict: {
      value(this: SimultaneousTrigger) {
        const currOrder = "order" in this && this.order ? this.order : {};
        (this as { order?: SimOrder }).order = { ...currOrder, down: "strict" };
        return this;
      },
      enumerable: false,
      writable: true,
    ...
    withOrder: {
      value(this: SimultaneousTrigger, orderSpec: SimOrder | "insensitive" | "strict" | "strict_inverse" | "uninterrupted") {
        const normalized = normalizeSimOrder(orderSpec);
        if (normalized) {
          const currOrder = "order" in this && this.order ? this.order : {};
          (this as { order?: SimOrder }).order = { ...currOrder, ...normalized };
        }
        return this;
      },
      enumerable: false,
    ...
    withModifiers: {
      value(this: SimultaneousTrigger, mods: TriggerModifiers) {
        (this as { modifiers?: TriggerModifiers }).modifiers = mods;
        return this;
      },
      enumerable: false,
      writable: true,
...
function isOrderArg(
  val: unknown,
): val is SimOrder | "insensitive" | "strict" | "strict_inverse" | "uninterrupted" {
 ...
 * - With timing threshold: `simultaneous("e", "k", 30)`
 * - With uninterrupted flag: `simultaneous("e", "k", "uninterrupted")`
 * - Chained fluent methods: `simultaneous("e", "k").uninterrupted().timing(30)`
 ...
 * - Configuration object: `simultaneous({ keys: ["j", "k"], uninterrupted: true, timing: 30 })`
 *
 * @param first - First key code, array of key codes, or a {@link SimultaneousTriggerOptions} configuration object.
 * @param rest - Remaining key codes, modifier array/object, order constraint, or ms timing threshold.
 * @returns A normalized {@link SimultaneousTrigger} representing the simultaneous chord with fluent builder methods.
 *
 * @example
 * ```ts
 ...
 *
 * // 2. Letter key chord with order & timing:
...
export function simultaneous(
  ...
  if (typeof first === "object" && !Array.isArray(first)) {
    ...
    const uninterrupted =
      typeof first.uninterrupted === "boolean"
        ? first.uninterrupted
        : typeof first.detect_key_down_uninterruptedly === "boolean"
          ? first.detect_key_down_uninterruptedly
          : typeof first.detectUninterrupted === "boolean"
            ? first.detectUninterrupted
            : undefined;
    const rawOrder = first.simultaneous_options ?? first.order;
    const orderObj: SimOrder | SimultaneousOptions =
      rawOrder === "uninterrupted"
        ...
        : typeof rawOrder === "string"
          ...
          : typeof rawOrder === "object" && rawOrder !== null
            ? { ...rawOrder }
            : {};
    if (uninterrupted !== undefined) {
      (orderObj as { detectUninterrupted?: boolean }).detectUninterrupted = uninterrupted;
    }
    const resolvedOrder = normalizeSimOrder(
      Object.keys(orderObj).length ? orderObj : undefined,
    );
    const rawMs =
      typeof first.simultaneousMs === "number"
        ? first.simultaneousMs
        : typeof first.thresholdMs === "number"
          ? first.thresholdMs
          : typeof first.threshold === "number"
            ? first.threshold
            : typeof first.timing === "number"
              ? first.timing
              : undefined;
    const timingObj: BindingTiming | undefined =
      typeof first.timing === "object" && first.timing !== null
        ? first.timing
        : rawMs !== undefined
          ? { simultaneousMs: rawMs }
          : undefined;

    return createSimultaneousTrigger(
      keysArray,
      first.modifiers,
      resolvedOrder,
      timingObj,
  ...
  let timing: BindingTiming | undefined;

  const processArg = (arg: unknown) => {
    if (typeof arg === "number") {
      timing = { ...(timing ?? {}), simultaneousMs: arg };
    } else if (isModifiersArg(arg)) {
      modifiers = arg;
    } else if (isOrderArg(arg)) {
      const norm = normalizeSimOrder(arg);
      if (norm) order = { ...(order ?? {}), ...norm };
    } else if (typeof arg === "object" && arg !== null) {
      const opts = arg as SimultaneousTriggerOptions;
      if (opts.modifiers) modifiers = opts.modifiers;
      const uninterrupted =
        typeof opts.uninterrupted === "boolean"
          ? opts.uninterrupted
          : typeof opts.detect_key_down_uninterruptedly === "boolean"
            ? opts.detect_key_down_uninterruptedly
            : typeof opts.detectUninterrupted === "boolean"
              ? opts.detectUninterrupted
              : undefined;
      const rawOrder = opts.simultaneous_options ?? opts.order;
      if (rawOrder || uninterrupted !== undefined) {
        const orderObj: SimOrder | SimultaneousOptions =
          rawOrder === "uninterrupted"
            ? { detectUninterrupted: true }
            : typeof rawOrder === "string"
        ...
        const norm = normalizeSimOrder(
          Object.keys(orderObj).length ? orderObj : undefined,
        );
        if (norm) order = { ...(order ?? {}), ...norm };
      }
      const rawMs =
        typeof opts.simultaneousMs === "number"
          ? opts.simultaneousMs
          : typeof opts.thresholdMs === "number"
            ? opts.thresholdMs
            : typeof opts.threshold === "number"
              ? opts.threshold
              : typeof opts.timing === "number"
                ? opts.timing
                : undefined;
      if (typeof opts.timing === "object" && opts.timing !== null) {
        timing = { ...(timing ?? {}), ...opts.timing };
      } else if (rawMs !== undefined) {
        timing = { ...(timing ?? {}), simultaneousMs: rawMs };
      }
    }
  };
  ...
  if (Array.isArray(first)) {
    ...
    return createSimultaneousTrigger(keysArray, modifiers, order, timing);
  }
 ...
 * @param order - Optional simultaneous key press order constraint (`"insensitive"`, `"strict"`, `"strict_inverse"`, `"uninterrupted"`, or {@link SimOrder}).
...
export function from(
  ...
  if (typeof input === "object" && input !== null) {
    ...
    if ("keys" in input && typeof (input as any).uninterrupted === "function") {
      const sim = input as SimultaneousTrigger;
      if (modifiers) sim.withModifiers(modifiers);
      if (order) sim.withOrder(order);
      return sim;
    }
    if ("simultaneous" in input || "chord" in input) {
      const sim = simultaneous(input as SimultaneousTriggerOptions);
      if (modifiers) sim.withModifiers(modifiers);
      if (order) sim.withOrder(order);
      return sim;
    }
    if ("keys" in input) {
      const simOpts =
        "simultaneous_options" in input ? (input as any).simultaneous_options : undefined;
      const rawOrder = "order" in input ? (input as any).order : order;
      const rawUninterrupted =
        typeof (input as any).uninterrupted === "boolean"
          ? (input as any).uninterrupted
          : typeof (input as any).detect_key_down_uninterruptedly === "boolean"
            ? (input as any).detect_key_down_uninterruptedly
            : undefined;
      const orderObj =
        rawOrder === "uninterrupted"
          ? { detectUninterrupted: true }
          : typeof rawOrder === "string"
            ...
            : typeof rawOrder === "object" && rawOrder !== null
              ? { ...rawOrder }
              : {};
      if (rawUninterrupted !== undefined) {
        orderObj.detectUninterrupted = rawUninterrupted;
      }
      const resolvedOrder = normalizeSimOrder(
        Object.keys(orderObj).length ? orderObj : simOpts,
      );
      const rawMs =
        typeof (input as any).simultaneousMs === "number"
          ? (input as any).simultaneousMs
          : typeof (input as any).thresholdMs === "number"
            ? (input as any).thresholdMs
            : typeof (input as any).threshold === "number"
              ? (input as any).threshold
              : typeof (input as any).timing === "number"
                ? (input as any).timing
                : undefined;
      const timingObj =
        typeof (input as any).timing === "object" && (input as any).timing !== null
          ? (input as any).timing
          : rawMs !== undefined
            ? { simultaneousMs: rawMs }
            : undefined;
      return triggerKeys(
        (input as any).keys as TriggerKey | TriggerKey[],
