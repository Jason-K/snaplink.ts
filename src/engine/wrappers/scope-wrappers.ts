...
export function withOptions(
  ...
  return flatten(blocks).map((b) => {
    ...
    const uninterrupted =
      typeof spec.uninterrupted === "boolean"
        ? spec.uninterrupted
        : typeof spec.detect_key_down_uninterruptedly === "boolean"
          ? spec.detect_key_down_uninterruptedly
          : undefined;
    if (uninterrupted !== undefined && "keys" in merged.trigger) {
      (merged.trigger as { order?: SimOrder }).order = {
        ...((merged.trigger as { order?: SimOrder }).order ?? {}),
        detectUninterrupted: uninterrupted,
      };
    }
    const simMs =
      typeof spec.simultaneousMs === "number"
        ? spec.simultaneousMs
        : typeof spec.thresholdMs === "number"
          ? spec.thresholdMs
          : undefined;
    if (simMs !== undefined) {
      merged.timing = { ...(merged.timing ?? {}), simultaneousMs: simMs };
    }
    return merged;
  });
}
 ...
 * or specifying a shared simultaneous chord window (in ms) across chord rules.
 *
 * @param profileOrTiming - Named {@link TimingProfileName} (`"snappy"`, `"deliberate"`, etc.), millisecond chord threshold number, or explicit timing threshold object.
 * @param blocks - One or more {@link BindingBlock} entries to configure.
 ...
 * ...withTiming(30, simultaneousBindings)
 * ```
 */
export function withTiming(
  profileOrTiming: number | TimingProfileName | NonNullable<Binding["timing"]>,
  ...blocks: BindingBlock[]
): Binding[] {
  if (typeof profileOrTiming === "number") {
    return withOptions({ timing: { simultaneousMs: profileOrTiming }, simultaneousMs: profileOrTiming }, ...blocks);
  }
