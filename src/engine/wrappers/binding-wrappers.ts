...
import type { anyInput, chord, simultaneous, trigger } from "./from-action-wrappers";
import type {
  actHere,
  app,
  appHistory,
...
export function timing(
  optsOrProfile: number | TimingProfileName | AcceptUndefined<NonNullable<Binding["timing"]>>,
  overrides?: AcceptUndefined<NonNullable<Binding["timing"]>>,
): OptionsWrapper {
  if (typeof optsOrProfile === "number") {
    return options({ timing: { simultaneousMs: optsOrProfile, ...(overrides ?? {}) }, simultaneousMs: optsOrProfile });
  }
...
/**
 * Creates an {@link OptionsWrapper} setting `detect_key_down_uninterruptedly: true` for simultaneous chords.
 *
 * Use in {@link bind} or {@link options} to ensure unrelated keys typed between chord keys will not cancel match.
 *
 * @param val - Whether uninterrupted detection is enabled (default `true`).
 * @returns An {@link OptionsWrapper} container.
 *
 * @example
 * ```ts
 * bind(simultaneous("e", "k"), to(tap(APPS.keSettings)), uninterrupted())
 * ```
 */
export function uninterrupted(val = true): OptionsWrapper {
  return options({ uninterrupted: val, detect_key_down_uninterruptedly: val });
}

/**
 * Alias for {@link uninterrupted}.
 */
export const detectKeyDownUninterruptedly = uninterrupted;

/**
 * Creates an {@link OptionsWrapper} setting the simultaneous chord detection threshold in milliseconds.
 *
 * @param ms - Threshold window in milliseconds (`basic.simultaneous_threshold_milliseconds`).
 * @returns An {@link OptionsWrapper} container.
 *
 * @example
 * ```ts
 * bind(simultaneous("e", "k"), to(tap(APPS.keSettings)), simultaneousTiming(30))
 * ```
 */
export function simultaneousTiming(ms: number): OptionsWrapper {
  return options({ simultaneousMs: ms, timing: { simultaneousMs: ms } });
}

/**
 * Alias for {@link simultaneousTiming}.
 */
export const threshold = simultaneousTiming;
 ...
 */
export type BindArg =
  ToWrapper | WhenWrapper | OptionsWrapper | Case | Case[] | Condition | Condition[] | BindingOptionsSpec;
...
export function bind(trigger: FromInput, ...args: BindArg[]): Binding {
  ...
  if ("chordTiming" in trg && trg.chordTiming) {
    mergedOptions.timing = { ...trg.chordTiming };
  }
  ...
  const uninterrupted =
    typeof mergedOptions.uninterrupted === "boolean"
      ? mergedOptions.uninterrupted
      : typeof mergedOptions.detect_key_down_uninterruptedly === "boolean"
        ? mergedOptions.detect_key_down_uninterruptedly
        : undefined;
  if (uninterrupted !== undefined && "keys" in trg) {
    (trg as { order?: SimOrder }).order = {
      ...((trg as { order?: SimOrder }).order ?? {}),
      detectUninterrupted: uninterrupted,
    };
  }

  const simMs =
    typeof mergedOptions.simultaneousMs === "number"
      ? mergedOptions.simultaneousMs
      : typeof mergedOptions.thresholdMs === "number"
        ? mergedOptions.thresholdMs
        : undefined;
  if (simMs !== undefined) {
    mergedOptions.timing = {
      ...(mergedOptions.timing ?? {}),
      simultaneousMs: simMs,
    };
  }
