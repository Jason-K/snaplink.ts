...
export function resolveSimKarOptions(b: Binding): KarSimultaneousOptions | undefined {
  const triggerOrder = "order" in b.trigger ? b.trigger.order : undefined;
  const uninterrupted =
    b.uninterrupted ??
    b.detect_key_down_uninterruptedly ??
    triggerOrder?.detectUninterrupted;

  const mergedOrder: SimOrder | undefined =
    triggerOrder || uninterrupted !== undefined
      ? {
          ...(triggerOrder ?? {}),
          ...(uninterrupted !== undefined ? { detectUninterrupted: uninterrupted } : {}),
        }
      : undefined;

  const order = resolveSimOrder(mergedOrder);
