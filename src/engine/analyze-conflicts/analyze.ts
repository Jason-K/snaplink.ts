...
function classify(
  ...
  // If they claim the exact same physical input but under conditions that
  // overlap without strict containment, whichever evaluates first will
  // silently swallow the input whenever both condition sets happen to hold.
  if (
    sameInputDomain(earlier.domain, later.domain) &&
    // A broader rule following a narrower rule is a narrowing, not an ambiguous overlap,
    // because it correctly acts as a fallback for the cases the narrower rule doesn't cover.
    !conditionsImply(later.conditions, earlier.conditions) &&
    // A narrower rule following a broader rule is also not an ambiguous overlap,
    // because we already flagged it as shadowed. Since shadowed check already
    // fired above, this should be redundant, but just in case:
    !conditionsImply(earlier.conditions, later.conditions) &&
    // Rules belonging to the same explicit ruleGroup (e.g. generated by a single builder
    // like modalLayer) are considered intentionally ordered by the author, and their
    // conditions are likely mutually exclusive in practice even if not provably so statically.
    !(
      earlier.binding.ruleGroup?.id &&
      earlier.binding.ruleGroup.id === later.binding.ruleGroup?.id
    )
  ) {
...
function messageFor(
  ...
  switch (kind) {
    ...
    case "ambiguous-overlap":
      return (
        `Ambiguous double-fire: ${a} (${describeConditions(earlier.conditions)}) and ` +
        `${b} (${describeConditions(later.conditions)}) match the exact same input, and ` +
